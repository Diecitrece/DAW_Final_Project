...
describe("API Books", () => {
    ...
    it("GET some Requests id and return this requests", async () => {
        const response = await axios.get("http://localhost:3000/api/requests/?id=62650f37a9f6bdc755282f2b");
        expect(response.data._id).toBe("62650f37a9f6bdc755282f2b");
    });
    it("POST pass some Requests name and return created requests", async () => {
        const response = await axios.post("http://localhost:3000/api/requests/", {name: "La guerra de las galaxias"});
        expect(response.status).toBe(200);
    });
